/*
	...
	REQUIREMENT:
		...
		At a minimum you should have :
			***A class for favorite thing ---> Sneaker.cpp
			***A array class that contains the collections of favorite things ---> SneakerCollection.cpp
		Your favorite thing class should contain:
			***At least 5 private member elements (data) (brand, name, color, ID/passcode, price)
		...
		Your array class should have :
			...
			***A method that checks if an item already exists. 
				The method should be passed a reference to a favorite thing object and return a boolean true/false (duplicate or not) value.
			***A method to Add an new item to the list when passed a reference to a favorite thing object 
				(note: you should have already determines that the item is not duplicate).
...
int main()
{
	...
	//do-while loop to allow user to input more favorite sneakers to the list 'till (s)he chooses to to exit 
	do
	{
		char another;
		myCollection.moreSneaker();
		//nested if statement to ask user to input more, and the execution will break/stop once the list exceeds its allowed size.   
		if (myCollection.getCollectionSize() >= 100)
		{
			moreSneaker = false;
			break;
		}
		// Continue to ask for user input as 'Y' or 'N' 
		cout << "\n\n\n			     Do you have another favorite sneaker? Hit 'Y' for yes" << endl;
		cout << "\n			     Otherwise, hit any key to exit out of the vault --->  ";
		cin >> another;
		cout << endl;
		// Letting user know that (s)he chose to add more sneaker to the list if 'Y' was entered, 
		...
		else 		// otherwise tell her/him that every input was saved
		{
			moreSneaker = false;
			cout << "\n\n\n		* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *" << endl;
